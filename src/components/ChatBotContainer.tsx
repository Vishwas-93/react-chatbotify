...
const ChatBotContainer = ({ flow }: { flow: Flow }) => {
   ...
   */
	...
	const [updatedChatWindowStyles, setUpdatedChatWindowStyles] = useState(getChatWindowStyle())
	...
	useEffect(() => {
		if (styles.chatWindowStyle) {
			setUpdatedChatWindowStyles((prev) => ({
				...prev,
				...styles.chatWindowStyle,
			}))
		}
   ...
   */
	const handleNotifications = useCallback(() => {
      ...
      message.sender === 'user' ||
      isBotTyping ||
      (settings.general?.embedded && isChatBotVisible(chatBodyRef.current as HTMLDivElement))
		) {
			return
		}
		...
		if (!settings.notification?.disabled && notificationToggledOn && hasInteractedPage && audioBufferRef.current) {
   ...
   */
	const injectMessage = useCallback(
		async (content: string | JSX.Element, sender = 'bot') => {
			const message = { content: content, sender: sender }
			processAudio(settings, audioToggledOn, message)

			const isBotStream =
        typeof message.content === 'string' && message.sender === 'bot' && settings?.botBubble?.simStream
			const isUserStream =
        typeof message.content === 'string' && message.sender === 'user' && settings?.userBubble?.simStream
   ...
   */
	const handleActionInput = useCallback(
		async (path: keyof Flow, userInput: string, sendUserInput = true) => {
   ...
   */
	...
	return (
		<div
			...
			<div style={updatedChatWindowStyles} className='rcb-chat-window'>
				...
				{settings.general?.showInputRow && (
					<ChatBotInput
						...
						hasFlowStarted={hasFlowStarted}
						setHasFlowStarted={setHasFlowStarted}
